...
import { QR_MARGIN } from '@/app/(page)/data/qr.data';
import { ErrorLevel } from '@/app/(page)/enums/qr.enums';
import { renderQrCanvas, renderQrSvg } from '@/app/(page)/helpers/qr.helpers';
import { useQrCode } from '@/app/(page)/hooks/qr.hooks';
import { triggerDownload } from '@/common/utils/download.functions';
 ...
 **/
export default function QrResult({ payload, level, size }: Props) {
  const { svg, moduleCount, error } = useQrCode(payload, level);
  const [copied, setCopied] = useState(false);

  const hasContent = payload.trim().length > 0;
  // The export carries the quiet zone too, so a square gets `size / (modules + margins)` pixels.
  const modulePixels = moduleCount ? Math.floor(size / (moduleCount + QR_MARGIN * 2)) : 0;

  const downloadPng = async () => {
    if (!hasContent) return;

    const canvas = await renderQrCanvas(payload, level, size);
  ...
  const downloadSvg = async () => {
    ...
    // Re-rendered, not reused: the export carries a fixed width/height, the preview stretches.
    const markup = await renderQrSvg(payload, level, size);
  ...
  return (
    <motion.section
          ...
          {hasContent && svg ? (
            ...
            <motion.div
              ...
              <p className='text-sm text-zinc-400 dark:text-zinc-500'>
                Fill in the form to generate a code
              </p>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {hasContent && moduleCount > 0 && (
        <p className='text-center text-xs text-zinc-400 dark:text-zinc-500'>
          {size} × {size} px export · {moduleCount} × {moduleCount} modules · {modulePixels} px per
          square
        </p>
      )}
